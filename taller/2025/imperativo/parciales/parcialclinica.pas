 ...
 begin
	if(a=nil)then
		...
		if(a^.dato.matri > m)then
			cantidadTotal:= a^.dato.cantAt+ cantidadTotal(a^.HI,m)+ cantidadTotal(a^.HD,m)
		else
			cantidadTotal:=cantidadTotal(a^.HD,m);
end;

//c) Realizar un módulo recursivo que reciba el vector generado en a) y retorne el diagnóstico con mayor cantidad de pacientes atendidos.
procedure contabilizar (l: listadni; var cant: integer);
begin
	cant:= 0;
	while (l <> nil) do begin
		cant:= cant +1;
...
begin
	if (dim>0)then begin
		...
		contabilizar(v[dim],cant);
		writeln('la cantidad de pacientes de la lista '  ,dim,' es: ' ,cant);
...
begin
	...
	if(maxCod=0)then
		writeln('Las listas estaban vacias')
	else
