  ...
  if (a <> nil) then begin
    if (a^.dato.codigo >= inf) then begin
      if (a^.dato.codigo <= sup) then begin
        contar:=contar + a^.dato.montoTotal ;
...
begin
  ...
  readln (codigo1);
  write ('Ingrese segundo codigo de producto (mayor al primer codigo): ', codigo1);//agregue yo para mostrar el codigo menor
  writeln();
  readln (codigo2);
  writeln;
  recorridoAcotado(a,codigo1,codigo2,montoTotal);
