...
procedure InformarExistenciaNombreSocio (a:arbol);
  ...
  {function encontre(a:arbol;nom:cadena15):boolean;
  begin
  if(a<>nil) then begin
    encontre:=encontre(a^.HI,nom);
    if(a^.dato.nombre=nom) then
      encontre:=true
    else
      encontre:=encontre(a^.HD,nom);
  end;
  end;}
  function encontre(a: arbol; nom: cadena15): boolean;
  begin
    if (a = nil) then
      encontre := false  (* Caso base: árbol vacío, el nombre no está *)
    else if (a^.dato.nombre = nom) then
      encontre := true  (* Si encontramos el nombre, devolvemos true *)
    else
      encontre := encontre(a^.HI, nom) or encontre(a^.HD, nom); (* Buscamos en ambos subárboles *)
  end;
var
nom:cadena15;
begin
  ...
  readln(nom);
  if (encontre(a,nom)) then
    writeln('Se encontraba el nombre en el arbol :) ')
...
procedure InformarCantidadSocios (a:arbol);
  function cantS(a:arbol):integer;
  begin
    if(a<>nil) then 
      cantS:=1+ cantS(a^.HI)+cantS(a^.HD)
    else
      cantS:=0;
...
tot:=cantS(a);
if (tot<>0) then
  writeln('La cantidad de socios es := ',tot)
...
procedure InformarPromedioDeEdad(a:arbol);
  ...
  procedure promedio(a:arbol; var edad:integer; var cant:integer);
  begin
  if(a<>nil) then begin
     promedio(a^.HI,edad,cant);
     cant:=cant+1;
     edad:=edad+a^.dato.edad;
     promedio (a^.HD,edad,cant);
   end;
end;
var 
 edad,cant:integer;
...
edad:=0;
cant:=0;
promedio(a,edad,cant);
writeln('el promedio de edades es:= ',(edad/cant):2:2);
end;
